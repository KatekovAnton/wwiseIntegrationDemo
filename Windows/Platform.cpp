...
void ShortenPath(AkOSChar* io_Path)
{
	...
	AkOSChar token[] = { AK_PATH_SEPARATOR[0] , '.','.'};
	const size_t tokensize = AKPLATFORM::OsStrLen(token);

	int searchIdx = 1;// Skip the case the path would start with the searched token.
	...
	while (searchIdx < len - tokensize)
	{
		...
		{
			int prevSeparatorIndex = searchIdx - 1; // The searchIdx is always greater than 1
