...
)
{
	...
	// Because we call AK::SoundEngine::Init with default AkInputSettings::settingsMainOutput, we need to divine
	// what the primary AkOutputDeviceId we have initialized with is. The Options menu will need this in order
	// to properly identify which AkOutputDeviceId should be Replaced when the Primary Output Device setting is modified
	m_outputDeviceId = AK::SoundEngine::GetOutputID(
		IntegrationDemo::GetDefaultAudioDeviceSharesetId(),
		IntegrationDemo::GetDefaultAudioDeviceId());
